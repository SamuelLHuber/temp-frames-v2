...
export const loader: LoaderFunction = async ({ request }) => {
  const signedmessagebytes = new URL(request.url).searchParams.get(
    "signedmessagebytes"
  );
  const action: boolean = new URL(request.url).searchParams.get("action")
    ? true
    : false;
  ...
  if (signedmessagebytes) {
    ...
    const { isValid, message } = await validateFrameSignature(
      signedmessagebytes
    );
    if (isValid) {
      ...
      if (actionTimestamp + 1000 * 60 * 5 < Date.now().valueOf()) {
        ...
        console.error(
          "actionTimestamp too old. Unix Action Timestamp:",
          actionTimestamp,
          "Now:",
          Date.now().valueOf()
        );
